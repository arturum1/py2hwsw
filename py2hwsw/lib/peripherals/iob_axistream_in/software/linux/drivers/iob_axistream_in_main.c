...
/* iob_axistream_in_main.c: driver for iob_axistream_in
 * using device platform. No hardcoded hardware address:
 * 1. load driver: insmod iob_axistream_in.ko
...
static struct iob_data iob_axistream_in_data = {0};
DEFINE_MUTEX(iob_axistream_in_mutex);

#include "iob_axistream_in_sysfs.h"

static const struct file_operations iob_axistream_in_fops = {
    .owner = THIS_MODULE,
    .write = iob_axistream_in_write,
    .read = iob_axistream_in_read,
    .llseek = iob_axistream_in_llseek,
    .open = iob_axistream_in_open,
    .release = iob_axistream_in_release,
};

static const struct of_device_id of_iob_axistream_in_match[] = {
    {.compatible = "iobundle,axistream_in0"},
    {},
};

static struct platform_driver iob_axistream_in_driver = {
    .driver =
        {
            .name = "iob_axistream_in",
            .owner = THIS_MODULE,
            .of_match_table = of_iob_axistream_in_match,
        },
    .probe = iob_axistream_in_probe,
    .remove = iob_axistream_in_remove,
};

//
// Module init and exit functions
//
static int iob_axistream_in_probe(struct platform_device *pdev) {
  struct resource *res;
  int result = 0;
  ...
  iob_axistream_in_data.regbase = devm_ioremap_resource(&pdev->dev, res);
  if (IS_ERR(iob_axistream_in_data.regbase)) {
    result = PTR_ERR(iob_axistream_in_data.regbase);
    goto r_ioremmap;
  }
  ...
  if ((iob_axistream_in_data.class =
           class_create(THIS_MODULE, IOB_AXISTREAM_IN_DRIVER_CLASS)) == NULL) {
...
r_dev_file:
  iob_axistream_in_remove_device_attr_files(&iob_axistream_in_data);
r_device:
  class_destroy(iob_axistream_in_data.class);
r_class:
  cdev_del(&iob_axistream_in_data.cdev);
r_cdev_add:
  unregister_chrdev_region(iob_axistream_in_data.devnum, 1);
r_alloc_region:
...
static int iob_axistream_in_remove(struct platform_device *pdev) {
  iob_axistream_in_remove_device_attr_files(&iob_axistream_in_data);
  class_destroy(iob_axistream_in_data.class);
  cdev_del(&iob_axistream_in_data.cdev);
  unregister_chrdev_region(iob_axistream_in_data.devnum, 1);
...
static int __init iob_axistream_in_init(void) {
  pr_info("[Driver] %s: initializing.\n", IOB_AXISTREAM_IN_DRIVER_NAME);

  return platform_driver_register(&iob_axistream_in_driver);
}

static void __exit iob_axistream_in_exit(void) {
  pr_info("[Driver] %s: exiting.\n", IOB_AXISTREAM_IN_DRIVER_NAME);
  platform_driver_unregister(&iob_axistream_in_driver);
}
...
static int iob_axistream_in_open(struct inode *inode, struct file *file) {
  pr_info("[Driver] iob_axistream_in device opened\n");
...
static int iob_axistream_in_release(struct inode *inode, struct file *file) {
  pr_info("[Driver] iob_axistream_in device closed\n");
...
static ssize_t iob_axistream_in_write(struct file *file, const char __user *buf,
  ...
  switch (*ppos) {
  case IOB_AXISTREAM_IN_SOFT_RESET_ADDR:
    ...
    break;
  case IOB_AXISTREAM_IN_ENABLE_ADDR:
...
static loff_t iob_axistream_in_llseek(struct file *filp, loff_t offset,
  ...
  if (new_pos < 0 || new_pos > iob_axistream_in_data.regsize) {
    return -EINVAL;
  }
...
module_init(iob_axistream_in_init);
module_exit(iob_axistream_in_exit);

MODULE_LICENSE("Dual MIT/GPL");
MODULE_AUTHOR("IObundle");
MODULE_DESCRIPTION("IOb-AXISTREAM-IN Drivers");
MODULE_VERSION("0.10");
