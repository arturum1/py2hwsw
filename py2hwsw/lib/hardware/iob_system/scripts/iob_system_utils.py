...
def iob_system_scripts(attributes_dict, params, py_params):
    ...
    if not params["internal_memories"]:
        update_system_h(attributes_dict)
...
def update_system_h(attributes_dict):
    """Update iob_system.h file to correct selection bits for no internal memories"""
    build_dir = attributes_dict["build_dir"]
    os.makedirs(f"{build_dir}/software/src", exist_ok=True)
    with open(f"{build_dir}/software/src/iob_soc_system.h", "w") as file:
        file.write("#This file was auto generated by iob_system_utils.py\n")
        file.write(
            """\
