...
module iob_axi_m_write #(
   ...
   // Global signals
   `include "iob_axi_m_write_iob_clk_s_port.vs"
   input rst_i,
   ...
   `include "iob_axi_m_write_axi_write_m_port.vs"
);
   ...
   iob_fifo_sync #(
      ...
      .ADDR_W  (FIFO_ADDR_W)
       ...
       // Global signals
       `include "iob_axi_m_write_iob_clk_s_s_portmap.vs"
       .rst_i           (rst_i),
       // Write port
   ...
   always @* begin
      ...
      case (w_state)
         ...
         default: begin  // WAIT_DATA_IN_FIFO
            if (!busy) begin
               if (w_resp_o != 2'b00) begin
                  w_state_nxt = WAIT_START;
               end else if (w_remaining_data_o > 0) begin
