 ...
 * 
 * This program validates that a UART core correctly implements the 16550 register map
 * by exercising the standard Linux 8250 serial driver through its termios API.
 */

#define TEST_PATTERN "IOb-UART16550 Loopback Test Pattern - 0123456789 - ABCDEFGHIJKLMNOPQRSTUVWXYZ"
 ...
 * This exercises the driver's ability to program the UART's LCR, DLL, and DLM registers.
 */
int configure_port(int fd, speed_t speed, int data_bits, int stop_bits, char parity, int flow_ctrl) {
    struct termios tty;
    ...
    if (flow_ctrl) tty.c_cflag |= CRTSCTS;
...
int main(int argc, char *argv[]) {
        ...
        for (int db = 7; db <= 8; db++) { 
            for (int sb = 1; sb <= 2; sb++) {
                ...
                for (int p = 0; p < 3; p++) {
                    ...
                    if (configure_port(fd1, baud_rates[b].speed, db, sb, parities[p], 0) < 0 ||
                        configure_port(fd2, baud_rates[b].speed, db, sb, parities[p], 0) < 0) {
                        printf("Config Error\n");
                        failure_count++;
                        continue;
                    }
    ...
    for (int i = 0; i < STRESS_SIZE; i++) stress_pattern[i] = (char)(i & 0xFF);
    ...
    if (configure_port(fd1, B115200, 8, 1, 'N', 1) == 0 &&
        ...
        if (run_transfer_test(fd1, fd2, TEST_PATTERN, strlen(TEST_PATTERN)) == 0) {
            ...
            printf("  HW Flow Control: FAIL (Check wiring or core RTS/CTS support)\n");
            failure_count++;
        }
    }
