...
// TODO: Do not know if this module supports concurrent access. I believe it
// does because no use of shared memory but have not tested it.
...
void module_vma_close(struct vm_area_struct *vma) {
  ...
  printk(KERN_NOTICE
         "Simple VMA close, virt %lx, Priv: %px Virt: %px Ord: %d\n ",
...
static int module_mmap(struct file *file, struct vm_area_struct *vma) {
  ...
  vma->vm_private_data =
      (void *)((uintptr_t)virtual_mem |
               (uintptr_t)
                   order); // Store order in the lower bits that are guaranteed
                           // to be zero because virtual_mem is page aligned
