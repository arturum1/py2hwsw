...
void shake128_inc_ctx_release(shake128incctx *state) { free(state->ctx); }
...
void shake256_inc_ctx_release(shake256incctx *state) { free(state->ctx); }
...
void shake128_ctx_release(shake128ctx *state) { free(state->ctx); }
 ...
 *              non-incremental, starts by zeroeing the state.
 *
 * Arguments:   - shake256ctx *state: pointer to (uninitialized) output Keccak
 *state
 *              - const uint8_t *input: pointer to input to be absorbed
 *                                            into s
 *              - size_t inlen: length of input in bytes
...
/** Release the allocated state. Call only once. */
void shake256_ctx_release(shake256ctx *state) { free(state->ctx); }
...
void sha3_256_inc_ctx_release(sha3_256incctx *state) { free(state->ctx); }
...
void sha3_384_inc_ctx_release(sha3_384incctx *state) { free(state->ctx); }
...
void sha3_512_inc_ctx_release(sha3_512incctx *state) { free(state->ctx); }
