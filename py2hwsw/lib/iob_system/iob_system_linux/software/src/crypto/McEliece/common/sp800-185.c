...
void cshake128_inc_init(shake128incctx *state, const uint8_t *name,
                        size_t namelen, const uint8_t *cstm, size_t cstmlen) {
...
void cshake128_inc_absorb(shake128incctx *state, const uint8_t *input,
                          size_t inlen) {
  shake128_inc_absorb(state, input, inlen);
}

void cshake128_inc_finalize(shake128incctx *state) {
  state->ctx[state->ctx[25] >> 3] ^= (uint64_t)0x04
                                     << (8 * (state->ctx[25] & 0x07));
  state->ctx[(SHAKE128_RATE - 1) >> 3] ^= (uint64_t)128
                                          << (8 * ((SHAKE128_RATE - 1) & 0x07));
  state->ctx[25] = 0;
}

void cshake128_inc_squeeze(uint8_t *output, size_t outlen,
...
void cshake256_inc_init(shake256incctx *state, const uint8_t *name,
                        size_t namelen, const uint8_t *cstm, size_t cstmlen) {
  uint8_t encbuf[sizeof(size_t) + 1];
...
void cshake256_inc_absorb(shake256incctx *state, const uint8_t *input,
                          size_t inlen) {
...
void cshake256_inc_finalize(shake256incctx *state) {
  state->ctx[state->ctx[25] >> 3] ^= (uint64_t)0x04
                                     << (8 * (state->ctx[25] & 0x07));
  state->ctx[(SHAKE256_RATE - 1) >> 3] ^= (uint64_t)128
                                          << (8 * ((SHAKE256_RATE - 1) & 0x07));
  state->ctx[25] = 0;
}

void cshake256_inc_squeeze(uint8_t *output, size_t outlen,
 ...
 **************************************************/
void cshake128(uint8_t *output, size_t outlen, const uint8_t *name,
               size_t namelen, const uint8_t *cstm, size_t cstmlen,
 ...
 *              - size_t inlen: length of input in bytes
 **************************************************/
void cshake256(uint8_t *output, size_t outlen, const uint8_t *name,
               size_t namelen, const uint8_t *cstm, size_t cstmlen,
               const uint8_t *input, size_t inlen) {
  shake256incctx state;
