...
int VersatSHATests() {
  ...
  String content =
      PushFileFromEthernet("../../software/KAT/SHA256ShortMsg.rsp");
  ...
  if (result.earlyExit) {
    printf("SHA early exit. Check testcases to see if they follow the expected "
           "format\n");
    return 1;
  }
...
int VersatAESTests() {
  ...
  if (result.earlyExit) {
    printf("AES early exit. Check testcases to see if they follow the expected "
           "format\n");
    return 1;
  }
...
int VersatMcElieceTests() {
  ...
  unsigned char *public_key =
      PushArray(globalArena, PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_PUBLICKEYBYTES,
                unsigned char);
  unsigned char *secret_key =
      PushArray(globalArena, PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_SECRETKEYBYTES,
                unsigned char);

  int versatTimeAccum = 0;

  String content =
      PushFileFromEthernet("../../software/KAT/McElieceRound4kat_kem.rsp");
  ...
  while (1) {
    ...
    // Software only implementation is slow and we are already comparing to KAT
    // anyway and so, for McEliece, we skipping software implementation test of
    // McEliece.
    // PQCLEAN_MCELIECE348864_CLEAN_crypto_kem_keypair(public_key, secret_key);

    unsigned char *public_key_hex = PushArray(
        globalArena, PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_PUBLICKEYBYTES * 2 + 1,
        char);
    unsigned char *secret_key_hex = PushArray(
        globalArena, PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_SECRETKEYBYTES * 2 + 1,
        char);

    GetHexadecimal(public_key, public_key_hex,
                   PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_PUBLICKEYBYTES);
    GetHexadecimal(secret_key, secret_key_hex,
                   PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_SECRETKEYBYTES);

    bool good = true;
    for (int i = 0; i < PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_PUBLICKEYBYTES;
         i++) {
    ...
    for (int i = 0; i < PQCLEAN_MCELIECE348864_CLEAN_CRYPTO_SECRETKEYBYTES;
         i++) {
