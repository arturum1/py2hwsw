...
def connect_peripherals_cbus(attributes_dict, peripherals, params):
    ...
    # Number of peripherals = peripherals
    num_peripherals = len(peripherals)
    ...
    for idx, peripheral in enumerate(peripherals):
        ...
        peripheral["connect"]["csrs_cbus_s"] = f"{peripheral_name}_cbus"
...
def generate_peripheral_base_addresses(
    ...
    n_subordinates_w = (len(peripherals_list) - 1).bit_length()
...
def generate_makefile_segments(attributes_dict, peripherals, params, py_params):
    ...
    with open(f"{build_dir}/software/auto_sw_build.mk", "w") as file:
        ...
        # Create a list with every unique peripheral name
        # NOTE: Ideally, the peripheral_name_list should contain the generated 'name' of peripherals (like iob_cache_axi) instead of the original 'core_name' of the peripheral (like iob_cache).
        ...
        if peripherals:
            ...
            file.write(
                "PERIPHERALS_INSTANCE_NAME ?="
                + " ".join([i["instance_name"] for i in peripherals])
                + "\n"
            )
...
"""
    ...
    with open(f"{build_dir}/hardware/fpga/auto_fpga_build.mk", "w") as file:
        ...
        # Set USE_INTMEM variable
        file.write(f"USE_INTMEM:={int(params['use_intmem'])}\n")
...
get_uut_build_deps:
	make -C $(ROOT_DIR)/$(RELATIVE_PATH_TO_UUT)/hardware/fpga build_deps USE_TESTER=1
	-cp $(ROOT_DIR)/$(RELATIVE_PATH_TO_UUT)/hardware/fpga/*.hex .

get_uut_run_deps:
	make -C $(ROOT_DIR)/$(RELATIVE_PATH_TO_UUT)/hardware/fpga run_deps USE_TESTER=1
	-cp $(ROOT_DIR)/$(RELATIVE_PATH_TO_UUT)/hardware/fpga/*.hex .
...
get_uut_build_deps:
	make -C $(ROOT_DIR)/$(RELATIVE_PATH_TO_UUT)/hardware/simulation build_deps USE_TESTER=1
...
"""
    ...
    with open(f"{build_dir}/software/src/auto_{name}_boot.lds", "w") as file:
        file.write("/* This file was auto generated by iob_system_utils.py */\n")
        file.write(
            f". = {hex((params['fw_baseaddr'] + (1 << params['fw_addr_w'])) - (1 << params['bootrom_addr_w']))};\n"
        )
